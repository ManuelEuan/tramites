...
class UsuarioTramiteAnalistaService
{
    static function AsignarTramite(Request $request){//Request $request
        try {
            ...
            if(!is_null($existe)){
                ...
                /*DB::insert('insert into tram_mdv_usuariotramite_analista ("USTR_NIDUSUARIOTRAMITE", "USUA_NIDUSUARIO", "USUA_NIDUSUARIOREGISTRO", "USUA_FECHA", "USTR_ACTIVO") 
    ...
    static function TramitesAnalista($USUA_NIDUSUARIO){//Request $request
        $query  = DB::table('tram_mdv_usuariotramite_analista')
                        ->where('USUA_NIDUSUARIO', $USUA_NIDUSUARIO)
                        ->where('USTR_ACTIVO', 1);
    ...
    static function AnalistaTramiteAsignado($USTR_NIDUSUARIOTRAMITE, $USUA_NIDUSUARIO){//Request $request
        $query  = DB::table('tram_mdv_usuariotramite_analista')
                        ->where('USTR_NIDUSUARIOTRAMITE', $USTR_NIDUSUARIOTRAMITE)
                        ->where('USUA_NIDUSUARIO', $USUA_NIDUSUARIO)
                        ->where('USTR_ACTIVO', 1);
