    ...
    <div class="container-fluid">
                ...
                <div class="row">
                    <div class="col-md-4">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" name="txtPalabraClave" id="txtPalabraClave"
        ...
        <div>
            ...
            <select style="width: 70px; display: inline-block;"
                class="custom-select custom-select-sm form-control form-control-sm" name="cmbNumeroRegistro"
                id="cmbNumeroRegistro">
                <option>10</option>
                <option>25</option>
                <option>50</option>
                <option>100</option>
        ...
        @if (count($data_tramite) > 0)
            <div id="tramite_servicio">
                <section>
                    @foreach ($data_tramite as $data)
                        <div class="card text-left" style="margin-bottom: 2rem;">
                            <div class="card-header text-primary titleCard">
                                <div class="row">
                                    <div class="col-10">
                                        {{ $data->TRAM_CNOMBRE }} <span
                                            class="badge badge-warning">{{ $data->UNAD_CNOMBRE }}</span>
                                    </div>
                                    <div class="col-2">
                                            ...
                                            <div style="float: right; color:#212529;">
                                                ...
                                                @if (Gate::allows('isAdministradorOrEnlace'))
                                                    @if ($data->TRAM_NIMPLEMENTADO > 0)
                                                        <a
                                                            href="javascript:TRAM_FN_CAMBIAR_ESTATUS({{ $data->TRAM_NIDTRAMITE_CONFIG }}, 0)"><i
                                                                class="fas fa-toggle-on sizeBtnIcon"
                                                                style="color:#28a745;"></i></a>
                                                    @elseif($data->TRAM_NIMPLEMENTADO == 0)
                                                        <a
                                                            href="javascript:TRAM_FN_CAMBIAR_ESTATUS({{ $data->TRAM_NIDTRAMITE_CONFIG }}, 1)"><i
                                                                class="fas fa-toggle-off sizeBtnIcon"></i></a>
                                                    @else
                                                    @endif
                                                @endif
                                                ...
                                                @if (!Gate::allows('isAdministradorOrEnlace'))
                                                    @if ($data->TRAM_NIMPLEMENTADO > 0)
                                                        <a style="pointer-events:none" title="Implementado"
                                                            href="javascript:void();"><i class="fas fa-circle sizeBtnIcon"
                                                                style="color:#28a745;"></i></a>
                                                    @elseif($data->TRAM_NIMPLEMENTADO == 0)
                                                        <a style="pointer-events:none" title="No implementado"
                                                            href="javascript:void();"><i class="fas fa-circle sizeBtnIcon"
                                    ...
                                    <div class="col-2">
                                        @if ($data->TRAM_NIDTRAMITE_CONFIG == null)
                                            <div style="float: right;">
                                                <a href="{{ route('gestor_configurar_tramite', ['tramiteID' => $data->TRAM_NIDTRAMITE, 'tramiteIDConfig' => 0]) }}"
                                                    class="btn btn-primary"
                                                    style="width:172px; float:right; margin-bottom:7px;">Configurar
                                                    trámite</a>
                                                <a href="{{ route('gestor_consultar_tramite', ['tramiteID' => $data->TRAM_NIDTRAMITE, 'tramiteIDConfig' => 0]) }}"
                                                    class="btn btn-primary" style="width:172px; float:right;">Ver
                                                    trámite</a>
                                            </div>
                                        @endif
                                    </div>
                            ...
                            @if ($data->TRAM_NIDTRAMITE_CONFIG > 0)
                                <div class="card-footer text-muted"
                                    style="background-color: transparent; border-top: none; border-bottom: none;">
                                    <span class="text-left" style="margin-right: 30px;">Creado:
                                        {{ date('d/m/Y', strtotime($data->TRAM_DFECHACREACION)) }}</span>
                                    <span class="text-left">Ultima Modificación:
                                        {{ date('d/m/Y', strtotime($data->TRAM_DFECHAACTUALIZACION)) }}</span>

                                    <div style="float: right;">
                                        <a href="{{ route('gestor_consultar_tramite', ['tramiteID' => $data->TRAM_NIDTRAMITE, 'tramiteIDConfig' => $data->TRAM_NIDTRAMITE_CONFIG]) }}"
                                            class="btn btn-primary" style="float: right;">Ver trámite</a>
                                    </div>
                                </div>
                            @endif
                        </div>
                    ...
                    <div>
                        <strong>Mostrando registros del
                            {{ ($data_tramite->currentpage() - 1) * $data_tramite->perpage() + 1 }}
                            al {{ $data_tramite->currentpage() * $data_tramite->perpage() }}
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        $(data.clasificacion).each(function(i, v) {
                            cmbClasificacion.append('<option value="' + v.id + '">' + v.name +
                                '</option>');
                        })
                        ...
                        $(data.dependencias).each(function(i, v) {
                            cmbDependenciaEntidad.append('<option value="' + v.id + '">' + v
                                .name + '</option>');
                        })
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        //data: $('#frmForm').serialize(),
                        //dataType: 'json',
                        url: '/gestores/implementar_tramite/' + TRAM_NIDTRAMITE_CONFIG + '/' +
                            TRAM_NIMPLEMENTADO + '',
                        type: "GET",
                        success: function(data) {
